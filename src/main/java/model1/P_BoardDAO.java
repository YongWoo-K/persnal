...
public class P_BoardDAO {
	...
	//write ok
	public int boardWriteOK(P_BoardTO to) {
		...
		try {
			...
			//데이터베이스에 데이터 집어넣기
			String sql = "insert into pet_board values (0,?,?,?,?,?,?,?,0,?,now())";
	...
	//list
	public ArrayList<P_BoardTO> boardList(){
		...
		ArrayList<P_BoardTO> lists = new ArrayList<P_BoardTO>();
		/*
		  ArrayList를 사용하는 이유
		    1. 원소를 추가/삭제할 수 있다.
		    2. null값을 가질 수 있다.
		...
		try {
			conn = dataSource.getConnection();
			String sql = "select seq, subject, filename, writer, date_format(wdate,'%Y-%m-%d') wdate, hit, datediff(now(),wdate)wgap from pet_board order by seq desc";
	...
	//paging list
	public P_BoardListTO boardList(P_BoardListTO listTO) {
		Connection conn = null; //연결해주는 객체를 생성
		//-> Connection = 연결
		PreparedStatement pstmt = null; //값을 나중에 입력한다.
		//-> PreparedStatement = 설정 및 실행
	...
	//view 디테일 또는 상세페이지
	public P_BoardTO boardView(P_BoardTO to) {
		Connection conn = null; //연결해주는 객체를 생성
		//-> Connection = 연결
		PreparedStatement pstmt = null; //값을 나중에 입력한다.
		//-> PreparedStatement = 설정 및 실행
		...
		try {
			...
			//조회수 증가시키기
			String sql = "update pet_board set hit = hit+1 where seq = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, to.getSeq());
			rs = pstmt.executeQuery();
		
			//데이터베이스에서 해당 글 내용 가져오기
			sql = "select subject, writer, mail, content, filename, hit, wip, wdate from pet_board where seq = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, to.getSeq());
			rs = pstmt.executeQuery();
	...
	//view 이전글
	public P_BoardTO boardView_before(P_BoardTO to_before) {
		Connection conn = null; //연결해주는 객체를 생성
		//-> Connection = 연결
		PreparedStatement pstmt = null; //값을 나중에 입력한다.
		//-> PreparedStatement = 설정 및 실행
		...
		try {
			...
			//데이터베이스에서 해당 글 내용 가져오기
			String sql = "select seq, subject from pet_board where seq = (select max(seq) from pet_board where seq < ?)";
			pstmt = conn.prepareStatement(sql);
	...
	//view 다음글
	public P_BoardTO boardView_next(P_BoardTO to_next) {
		Connection conn = null; //연결해주는 객체를 생성
		//-> Connection = 연결
		PreparedStatement pstmt = null; //값을 나중에 입력한다.
		//-> PreparedStatement = 설정 및 실행
		...
		try {
			...
			//데이터베이스에서 해당 글 내용 가져오기
			String sql = "select seq, subject from pet_board where seq = (select min(seq) from pet_board where seq > ?)";
			pstmt = conn.prepareStatement(sql);
	...
	//삭제할 글의 내용 가져오는 메소드
	public P_BoardTO boardDelete(P_BoardTO to) {
		Connection conn = null; //연결해주는 객체를 생성
		//-> Connection = 연결
		PreparedStatement pstmt = null; //값을 나중에 입력한다.
		//-> PreparedStatement = 설정 및 실행
		...
		try {
			...
			String sql = "select subject, writer from pet_board where seq = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, to.getSeq());
			rs = pstmt.executeQuery();
	 ...
	 */
	public int boardDeleteOK(P_BoardTO to) {
		Connection conn = null; //연결해주는 객체를 생성
		//-> Connection = 연결
		PreparedStatement pstmt = null; //값을 나중에 입력한다.
		//-> PreparedStatement = 설정 및 실행
		...
		try {
			conn = dataSource.getConnection();
			String sql = "select filename from pet_board where seq = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, to.getSeq());
			rs = pstmt.executeQuery();
			...
			sql = "delete from pet_board where seq = ? and password = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, to.getSeq());
	...
	public P_BoardTO boardModify(P_BoardTO to) {
		Connection conn = null; //연결해주는 객체를 생성
		//-> Connection = 연결
		PreparedStatement pstmt = null; //값을 나중에 입력한다.
		//-> PreparedStatement = 설정 및 실행
		...
		try {
			...
			String sql = "select writer, subject, content, mail, filename from pet_board where seq = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, to.getSeq());
			rs = pstmt.executeQuery();
	...
	public int boardModifyOK(P_BoardTO to) {
		Connection conn = null; //연결해주는 객체를 생성
		//-> Connection = 연결
		PreparedStatement pstmt = null; //값을 나중에 입력한다.
		//-> PreparedStatement = 설정 및 실행
		...
		try {
			...
			if(to.getFilename() != null) {
				sql = "update pet_board set subject = ?, content = ?, mail = ?, filename = ? where seq = ? and password = ?";
				pstmt = conn.prepareStatement(sql);
			...
			//수정 과정에서 첨부파일이 없을 경우
				...
				sql = "update pet_board set subject = ?, content = ?, mail = ? where seq = ? and password = ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, to.getSubject());
				pstmt.setString(2, to.getContent());
				pstmt.setString(3, to.getMail());
			...
			if(result == 0) {
				...
				if(to.getFilename() != null && oldfilename != null) {
					File file = new File("D:/dev_kyw/jsp_lesson/pet/src/main/webapp/upload/" + oldfilename);
					file.delete();
				}
			}
		}catch(SQLException e) {
